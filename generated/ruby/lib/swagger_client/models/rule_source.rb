  ...
  class RuleSource
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::RuleSource` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::RuleSource`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'channel_filter')
        self.channel_filter = attributes[:'channel_filter']
      end
    ...
    def list_invalid_properties
      ...
      if @channel_filter.nil?
        invalid_properties.push('invalid value for "channel_filter", channel_filter cannot be nil.')
      end
    ...
    def valid?
      return false if @channel_filter.nil?
